...
def Canvas_tree(
    ...
    max_rmsd = tree[2]
    if force_max_rmsd is not None:
        max_rmsd = max(force_max_rmsd, max_rmsd)
        top_rmsd = tree[2]
        if top_rmsd<max_rmsd:
            # add another "final" edge running out to max_rmsd
            center = Center(tree,node_position,sizes,use_sizes_as_weights=True)
            score = tree[3]
            size = Size(tree,sizes)
            edges.append([[top_rmsd,center],[max_rmsd,center], score, size, -1])
    min_rmsd = tree[2]
    for e in edges:
    ...
    if show_rmsd_bar: ## show scale
        ...
        if show_colorful_rmsd_bar:
            ...
            line_ypad = 5
            line_ypad = 2
            plotter.make_line([ line_x0,y0+line_ypad],[ line_x1,y0+line_ypad],2,0.0,
                              color='black' )


        label_start, label_stop = int(floor(min_rmsd+0.99)), 1+int(floor(max_rmsd+0.99))
        for i in range(label_start, label_stop, rmsd_bar_label_stepsize):
